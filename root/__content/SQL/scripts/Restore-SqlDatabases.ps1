...
if (($restore_dbs -ne $null) -and ($restore_dbs -ne "")) {
	...
	if ($null -ne $rdbs -And $rdbs.Length -gt 0) {
		...
		& (Join-Path $currDir Attach-AllDatabasesInDir.ps1) -server $server -DatabaseDir $data_db_folder
		...
		{	
			...
			if ($serverDb -eq $null) 
			{
				...
				foreach ($file in $restore.ReadFileList($server)) 
				{
					...
					{
						$relocateFile.PhysicalFileName = "$log_db_folder\$dbname.ldf";
					}
				...
				try
				{
					...
					& (Join-Path $currDir Attach-AllDatabasesInDir.ps1) -server $server -DatabaseDir $data_db_folder -DbNameToAttach $dbname
				}
				catch
				{
